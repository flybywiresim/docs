...
If we depart prior to the ETT, it will be automatically deleted and the predictions will be updated automatically based on the current time, as usual. On the other hand, if we don't depart by 12:30Z, the "CLK IS TAKE OFF TIME" scratchpad message appears on the MCDU as a reminder. At this point, time predictions are continuously updated based on the current time. We can insert a new ETT by repeating the whole process.
