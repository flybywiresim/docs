...
All actuators of the plane are fully physically modeled, and will react to solenoid valve command signals. Good news is that all of those signals are simulated realistically by the new flight computers implementation, which means the whole flight controls and hydraulic system now forms a cohesive set, able to simulate the behaviours of the real plane, including complex failure modes.
