...
Further tweaks and refinements have been implemented with this release. We have notably fixed various issues related to holds, turn directions for course / path capture 
transitions, alongside fixes and support for different leg types. We've also improved the performance when loading various procedures
...
We have released our custom-built solution SimBridge, which enables our users to connect various external devices and data for use with MSFS. 
