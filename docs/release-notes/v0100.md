...
Alternate control can be utilized via the RMP to enable backup tuning by selecting the NAV button. This will cause the RAD NAV page on the MCDU to go blank, and the FMGC will no longer tune any NAVAIDs.

The ILS frequency is automatically tuned for the departure runway and when the aircraft is within 250 NM of the destination airport. Scratchpad warning messages will appear if the ILS or ILS CRS 
...
200 NM of the destination. Constraints for waypoints behind the aircraft have also been added. 
