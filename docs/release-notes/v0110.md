...
!!! tip "Recommended Settings"
    ...
    **SimBrief Integration**
    ...
    We have streamlined the authentication process with Navigraph services (SimBrief included). Please note any changes on the following page:
...
Our latest flyPadOS brings forward several key enhancements and fixes that pilots can now enjoy.
...
!!! tip ""
    ...
    **EFB/FWC Update**
    ...
    Automatic Call-Outs are now selectable in the flyPad. The basic configuration offered by Airbus is set as the default.
    ...
    **EFB/Payloads Enhancements**
    ...
    !!! warning "Lack of Animation"
        Please note that due to Asobo model issues the animation of the doors will not be visible. Use the EFB or lower ECAM to verify the door status of the aircraft. 
...
Pilots are now able to initiate an engine start from crossbleed air. This procedure is normal performed when the APU Bleed start is not possible. For more information and 
procedure, visit our dedicated [Crossbleed Engine Start](../pilots-corner/advanced-guides/engines/crossbleed-start.md) page.
...
<!--TODO add link to ACSC page requires PR merge and rebase-->
...
- System Failures: A crucial aspect of any simulation is the unpredictability and the challenges it presents. Pilots can now set failures in various components. See our 
...
We have even built our own flight data recorder analysis tool that allows us to parse the data you've sent us and visualize it in a way that allows us to quickly identify the 
issues above. 
...
enhancements include the transmission of annunciator light state, display, integral light brightness, and support for BRT/DIM keys. A few bugs, such as the FMGC annunciator light not illuminating and demanding QNH and TEMP readings when distant from the destination, have been addressed. The ZFW/ZFWCG autofill feature has been optimized to use desired values during the boarding process.
