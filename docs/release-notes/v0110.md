...
Immerse yourself with advanced navigationa capabilities as we introduce vertical navigation to the Stable Version of the A32NX. We have refined various crucial flight 
...
Don't just take our word for it; step into the cockpit and witness the magic firsthand. Happy flying!
...
!!! tip ""
    ...
    **EFB/FWC Update**
    ...
    Automatic Call Outs are now selectable in the flyPad. The basic configruation offered by Airbus is set as the default.

    **EFB/Payloads Enhancements**

    Pilots now have the capability for multi-gate boarding whenever multiple doors are open, streamlining the boarding process and saving time.
    ...
    **EFB Functional Upgrades**
    ...
    Main dashboard and landing calculator have been refined to use your selected METAR source for a consistent weather information feed.
...
We have now included secondary engine parameters into the virtual Flight Data Recorder (FDR).

While most not be aware of this little fact, is that we do actually have an onboard FDR in the A32NX. This is a crucial component in the aircraft that records and logs various parameters and events. This data in real life is then used to analyze and investigate any incidents or accidents that may occur. 

For the purposes of simulation we use it for providing technical support to our users and analyzing the same parameters and events. It allows us to troubleshoot issues like a drifting flight stick or if the aircraft was stalled by pilot error or a bug in the flight model.

We have even built or own flight data recorder analysis tool that allows us to parse the data you've sent us and visualize it in a way that allows us to quickly identify the issues above. 

!!! tip "Snapshotting an Event"
    You can use the DFDR button found on the lower pedestal to mark a point in time within the recorded data so that we can quickly find the event you're reporting.

    See [here](../../pilots-corner/a32nx-briefing/flight-deck/pedestal/lighting-aids-dfdr).
...
!!! tip ""
    ...
    **SD**
    ...
    System Display (SD) updates involve a new 3-second display duration for the STS page. The SD BLEED page now correctly shows the APU bleed pressure indication. Corrections have been made in the fuel transfer valve amber logic, the position of outer tank fuel quantity values, and adjustments to the thickness and position of lower ECAM fuel page lines.

    **ISIS**

    A fix has been implemented for the ISIS alt tape, ensuring correct order representation for negative altitudes.
...
Layout adjustments have been made to the PERF CLB, PERF CRZ, and PERF DES pages for better clarity. Additionally, the robustness of ILS selection has been improved, and procedures referencing runways are now loaded correctly. Other enhancements include displaying ILS identification and frequency on the ARRIVAL page, improved fuel warnings post-engine start, and appropriate use of station declination.

The MCDU has also seen multiple updates. This includes basic MCDU MENU functionality, annunciator support, and improvements in V-speed calculations. Remote MCDU protocol 
enhancements include the transmission of annunciator light state, display, integral light brightness, and support for BRT/DIM keys. A few bugs, such as the FMGC annunciator light not illuminating and demanding QNH and TEMP readings when distant from the destination, have been addressed. The ZFW/ZFWCG auto-fill feature has been optimized to use desired values during the boarding process.
