...
??? note "What is the blue dashed line?"
    You might ask what the blue dashed line is. It is part of the new FIX INFO feature available only in the Experimental version for the time being. It allows to draw distance rings or radial lines from navigation fixes. For this illustration it helps us to visualize the localizer signal path at 261° into the runway which we are going to capture with the APPR mode.
