...
!!! warning "Development in progress"
    The Hoppie ACARS system is a complex system and we have decided to develop and release it step-by-step. So please be aware the not all features are available in the early releases and that we will add additional functionality in the future.

    The currently planned future releases:
...
The Hoppie system is developed by Jeroen "Hoppie" Hoppenbrouwers which provides text-based communication between aircrafts, virtual airlines and virtual ATC stations. It provides multiple communication protocols to communicate between different stations. The system is used by controllers on VATSIM and IVAO.
...
!!! note "[CPDLC](https://skybrary.aero/articles/controller-pilot-data-link-communications-cpdlc) stands for Controller Pilot Data Link Communication."
...
CPDLC is used to request changes from the ATC station or the ATC station requests flight information or gives instructions to the pilot.
...
!!! warning "CPDLC Availability"
    ...
    Also the network is not always reliable. Repeat your request via voice when in doubt. See [Troubleshooting](#troubleshooting).

### Create a logon code

Every pilot needs a unique logon code that is used to identify the communication.
This code needs to be requested on the website of Hoppie: [Logon request](http://www.hoppie.nl/acars/system/register.html){target=new}
...
A possible reason for a freetext can be the inability for specific SIDs or the request of a dedicated SID.
...
On ATC CTR sectors it is possible to use CPDLC. It is not provided on the ground or in the approach or departure sectors. Due to the long delays in the communication via CPDLC is this type of communication dedicated to the enroute stations.

#### Station Handover / Notification

If the responsible ATC sector provides CPDLC a notification is required. This notification requests a logon at the defined ATC station. Before a notification request is sent a check-in via voice mandatory, otherwise will the request be rejected.
...
The controller can send instructions to the pilot.
...
If you select STBY the message will be marked with `STDBY` and we can respond with `WILCO` or `UNABLE` after we have checked our ability to comply.
...
Sometimes can it happen that the ATC station does not react on PDC or CPDLC message. This can happen due to aircraft, ATC or Hoppie issues.
